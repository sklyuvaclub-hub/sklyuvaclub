...
async function loadBlogs() {
    ...
    try {
            ...
            <article class="blog-card">
                ...
                <div class="blog-content">
                    ...
                    <div class="blog-actions">
                        <button type="button" class="blog-share-btn" data-blog-id="${escapeHTML(blog.id)}" aria-label="Share this blog">
        ...
        if (!blogsContainer.dataset.blogActionsBound) {
            blogsContainer.addEventListener('click', async event => {
                const shareButton = event.target.closest('.blog-share-btn');
                if (shareButton) {
                    const blog = blogs.find(item => item.id === shareButton.dataset.blogId);
                    if (!blog) return;
                    const shareUrl = `${window.location.origin}${window.location.pathname}?blog=${encodeURIComponent(blog.id)}`;
                    const shareData = {
                        title: blog.title || 'SKL Yuva Club Blog',
                        text: blog.content || blog.title || 'SKL Yuva Club Blog',
                        url: shareUrl
                    };
                    try {
                        if (navigator.share) {
                            ...
                            window.prompt('Copy this blog link:', shareUrl);
                        }
                    } catch (error) {
                        if (error?.name !== 'AbortError') console.error('Blog Share Error:', error);
                    }
                    return;
                }
            ...
            blogsContainer.dataset.blogActionsBound = 'true';
        }
    } catch (error) {
