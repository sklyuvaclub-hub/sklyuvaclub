...
const HOME_POSITION_ORDER = {
    "President": 1, "Vice President": 2, "Secretary": 3,
    "Vice Secretary": 4, "Cashier": 5, "Vice Cashier": 6, "Adviser": 7
};
function homePositionOrder(position) {
    return HOME_POSITION_ORDER[String(position || "").trim()] ?? 999;
}
...
async function loadHomeMembers() {
    ...
    try {
        ...
        members.sort((a, b) => {
            const positionDiff = homePositionOrder(a.position) - homePositionOrder(b.position);
...
async function loadDynamicHero() {
    ...
    // Show the static hero immediately. Firebase content updates it in the background.
    ...
    try {
        ...
        // Preload the first Firebase image, but keep the current hero visible meanwhile.
        ...
        slider.innerHTML = slides.map((slide, index) => `
            <div class="slide ${index === 0 ? "active" : ""}">
                <img src="${escapeHero(slide.imageUrl)}" alt="${escapeHero(slide.title || "")}" loading="${index === 0 ? "eager" : "lazy"}" fetchpriority="${index === 0 ? "high" : "auto"}" decoding="async">
            </div>
        `).join("");
        ...
        // Hero is already visible; Firebase content is now applied.
